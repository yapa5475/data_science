...
def bigrams(sentence):
    ...
    bigram - pair of consecutive letters
    """
...
class BigramLanguageModel:
    ...
    def generate(self, context):
        ...
        for word in self._vocab:
            probabilitymodel = self.laplace(context, word)
            if(probabilitymodel > max):
                next_word = word
                max = probabilitymodel
        return next_word
    ...
    def laplace(self, context, word):
        ...
        if word not in self.contextCounts[context]:
            overall_vocab = self._obs_counts[context][word] + 1
            list1 = 0
            for item in self._obs_counts.items():
                if ww[0][0] == context:
                    list1 = list1 + ww[1]

        overall_vocab = len(self._vocab)
        total = list1 + overall_vocab
        prob = overall_vocab/total
    ...
    def add_train(self, sentence):
        ...
        for context, word in bigrams(list(self.tokenize_and_censor(sentence))):
            ...
            # ---------------------------------------
            assert word in self._vocab, "%s not in vocab" % word
            self._obs_counts.update({(context,word)})
            '''

...
class BigramLanguageModel:
    ...
    def generate(self, context):
        ...
        for k in self._vocab:
            prob_model = self.laplace(context, k)
            if(prob_model > max):
                word_after = k
                max = prob_model
        return word_after        
    ...
    def laplace(self, context, word):
        ...
        occurence = self.thecounter[context,word] + 1
        lst = 0
        ...
        total_vocab = len(self._vocab)
        total = lst + total_vocab
        probability = occurence / total
    ...
    def add_train(self, sentence):
        ...
        for context, word in bigrams(list(self.tokenize_and_censor(sentence))):
            ...
            # ---------------------------------------
            assert word in self._vocab, "%s not in vocab" % word
            self.thecounter.update({(context, word)})

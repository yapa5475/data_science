...
class LogReg:
    ...
    def sg_update(self, train_example):
        ...
        update_pi = (train_example.y - pi)*(train_example.x)
        
        self.beta = self.beta + (self.learning_rate*update_pi)
...
if __name__ == "__main__":
    ...
    #find most influention baseball word
    mxIndx, mxVal = max(enumerate(lr.beta), key=operator.itemgetter(1))
    print(mxIndx)
    print(vocab[mxIndx])

    print("\n")

    #find most influentional hockey word
    minIndx, minVal = min(enumerate(lr.beta), key=operator.itemgetter(1))
    print(minIndx)
    print(vocab[minIndx])

    print("\n") 

    #find least infl baseball word
    


    #find least infl. hockey word
    #tl2 = lr.beta
    #for index, value in enumerate(tl2):
